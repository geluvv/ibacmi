...
// Load required files first - BEFORE try block
...
// Use statements must be at the top level - BEFORE try block
...
try {
    ...
    // Check for 'users' table specifically (this is the admin table)
    $adminTable = null;
    foreach (['users', 'admin_users', 'admins', 'admin', 'tbl_admin', 'tbladmin'] as $tableName) {
    ...
    if (!$adminTable) {
        throw new Exception("Admin table not found");
    }
    
    // Use the found table name (should be 'users')
    $stmt = $db->prepare("SELECT * FROM `$adminTable` WHERE email = ? LIMIT 1");
    if (!$stmt) {
    ...
    $userId = null;
    $possibleIdColumns = ['id', 'admin_id', 'user_id', 'AdminID', 'ID'];
    ...
    $username = 'Admin';
    $possibleNameColumns = ['username', 'name', 'fullname', 'full_name', 'Username', 'Name', 'first_name'];
    ...
    $stmt = $db->prepare("INSERT INTO password_reset_tokens (user_type, user_id, email, token, expires_at) VALUES ('admin', ?, ?, ?, ?)");
    if (!$stmt) {
    ...
    $resetLink = $protocol . "://" . $host . "/ibacmi/AdminAccount/reset-password.html?token=" . $token;
    ...
    try {
        ...
        // Content
        $mail->isHTML(true);
        $mail->Subject = 'Password Reset Request - IBACMI Admin';
        $mail->Body = "
        <html>
        <head>
            <style>
            ...
            <div class='email-wrapper'>
                <div class='container'>
                    <div class='header'>
                        ...
                        <p>IBACMI Registrar Document Online Data Bank - Admin</p>
                    </div>
                    <div class='content'>
                        <p class='greeting'>Hello <strong>" . htmlspecialchars($username) . "</strong>,</p>
                        <p class='message'>
                            We received a request to reset your password for your <strong>IBACMI Admin Account</strong>. 
        ...
        $mail->AltBody = "Hello $username,\n\nWe received a request to reset your password for your IBACMI Admin Account.\n\nReset your password by visiting this link:\n$resetLink\n\nThis link will expire in 1 hour.\n\nIf you didn't request this, please ignore this email.\n\nIBACMI ORDDB System";

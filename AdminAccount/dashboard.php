<?php
// filepath: c:\xampp\htdocs\ibacmi\AdminAccount\dashboard.php
...
// ✅ AUTOMATIC ARCHIVAL CHECK - Runs silently on dashboard load
// This checks if any students are eligible for archival and archives them automatically
try {
    // Only check once per hour to avoid excessive processing
    $lastCheckFile = __DIR__ . '/last_archival_check.txt';
    $shouldCheck = true;
    
    if (file_exists($lastCheckFile)) {
        $lastCheck = (int)file_get_contents($lastCheckFile);
        $hoursSinceCheck = (time() - $lastCheck) / 3600;
        $shouldCheck = $hoursSinceCheck >= 1; // Check every 1 hour
    }
    ...
    if ($shouldCheck) {
        ...
        // Trigger archival check asynchronously (non-blocking)
        $checkUrl = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http') . 
                    '://' . $_SERVER['HTTP_HOST'] . 
                    dirname($_SERVER['SCRIPT_NAME']) . '/check_archival.php';
    ...
    // Silent fail - don't break dashboard if archival check fails
    error_log("Archival check error: " . $e->getMessage());
}
...
<body>
    <div class="container-fluid p-0">
        <?php include 'sidebar.php'; renderSidebar('dashboard'); ?>

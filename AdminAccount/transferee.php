...
error_log("=== TRANSFEREE.PHP LOADED ===");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['action'])) {
    error_log("=== TRANSFEREE FORM PROCESSING STARTED ===");
    ...
    try {
        ...
        echo json_encode([
            'status' => 'success',
            'message' => 'Transferee registered successfully',
...
$htmlContent = file_get_contents(__DIR__ . '/transferee.html');
...
if (preg_match('/<body[^>]*>(.*?)<\/body>/is', $htmlContent, $bodyMatches)) {
    ...
    if ($isStaffView) {
        $bodyContent = str_replace('action="transferee.php"', 'action="stafftransferee.php"', $bodyContent);
        $bodyContent = str_replace("action='transferee.php'", "action='stafftransferee.php'", $bodyContent);
    }
}

// Extract ALL scripts EXCEPT inline ones (we'll use external transferee.js instead)
$externalScripts = '';
...
<head>
    ...
    <title><?php echo $isStaffView ? 'Transferee - Staff' : 'Transferee - Admin'; ?> - IBACMI</title>
...
<body>
    <div class="container-fluid p-0">
        ...
        if ($isStaffView) {
            ...
            renderSidebar('transferee', 'staff', $staffInfo);
        } else {
            include 'sidebar.php';
            renderSidebar('transferee');
        }
        ?>
    ...
    <!-- Main transferee functionality -->
    <script src="<?php echo $isStaffView ? '../AdminAccount/' : ''; ?>js/transferee.js"></script>
